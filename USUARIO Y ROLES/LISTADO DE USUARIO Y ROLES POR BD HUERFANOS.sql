-- *** LISTADO DE USUARIO Y ROLES POR C/BD huerfanos***
...
BEGIN
   ...
   BEGIN
      ...
      SELECT @SQLquery = @SQLquery + ' sid NOT IN (SELECT sid FROM master.dbo.syslogins) '
...
LEFT JOIN master..syslogins lo
	ON us.sid = lo.sid
--where lo.name LIKE 'PERU'
--where dbuser LIKE 'PERU'

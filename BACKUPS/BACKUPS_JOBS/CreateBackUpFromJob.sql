...
---------------------------------------------------------------------------------------
    ...
    WHILE ( @@FETCH_STATUS = 0 ) 
        BEGIN
            -- LINEA ORIGINAL
            --SET @EchoCommand = 'ECHO ' + @EchoField + ' >> ' + @Path + @JobName + REPLACE(REPLACE(CONVERT(VARCHAR(200), GETDATE())  , ' ',  ''), ':', '') + '.txt'
            --SET @EchoCommand = 'ECHO ' + @EchoField + ' >> ' + REPLACE(@Path,' ','_') + @JobName + '.txt'
            SET @EchoCommand = 'ECHO ' + @EchoField + ' >> ' + @Path + REPLACE(REPLACE(REPLACE(RTRIM(LTRIM(@JobName)),' ','_'),'  ',''),'&','and') + '.txt' 
    --Print @EchoCommand
...
--SELECT replace('   Sample Hola  ', ' ', '');   
--•Resultado:'SampleHola'  
